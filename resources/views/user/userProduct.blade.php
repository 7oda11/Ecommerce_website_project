    ...
    <div class="container p-3">
        ...
        <div class="row justify-content-center">
            <div class="col-8" >
                ...
                <a href="{{ route('product') }}" class="btn btn-warning d-block w-100 mb-3"> Reset</a>
            </div>
        </div>
        ...
        <div class="row row-cols-1 row-cols-md-3 g-3">
            @if (count($userproducts) === 0)
                <div class="col w-100">
                    <p class="text-center text-bold fs-3 alert alert-danger">No products that have the same name available or you do not have any product in your cart</p>
                </div>
            @else
                    ...
                    <div class="col">
                        <div class="card h-100">
                            ...
                            <div class="card-footer">
                                <form action="{{ route('user.show', $product->id) }}" method="get">
                                ...
                                <form action="{{ route('delete', $product->id) }}" method="post">
                                    @csrf
                                    @method('delete')
                                    <button type="submit" class="btn btn-danger d-block w-100 mb-1"> Delete</button>
                                </form>
                            </div>
                        </div>
                    </div>
